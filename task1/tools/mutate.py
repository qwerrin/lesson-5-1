...
MUTATIONS: list[tuple[str, str, str, str]] = [
    ...
    # ---------------------------------------------------------- 「必要に応じて」
    # **実装を直したら壊しかたも直す。** ここは当初 ``if not messages`` の
    # 早期 return を狙っていたが、その行は「外しても結果が変わらないガード」
    # だったので実装から消した。壊しかたを直し忘れると「置換先なし」になり、
    # **何も壊さずに全部通って「穴ゼロ」に見える**（だから NOT FOUND は
    # 素通りと同じ扱いにしてある）。
    (
        SUM,
    ...
    # **無効なミューテーションを置かない。** ここは最初
    # 「``summary = None`` を足す」形だったが、直後に return しているので
    # 動作が1ミリも変わらず、**素通りとして報告された**（2026-08-29）。
    # 「テストが守っていない」と「壊せていない」は別の事実で、混ぜると
    # テストの穴を誤診する。壊すなら**経路が変わる形**で壊す。
    (
        TOOL,
